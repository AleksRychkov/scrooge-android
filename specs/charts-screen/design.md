...
Track whether currency selection is automatic or manual in internal filter state. On initialization and Reset, enter automatic mode and resolve the most-used currency for the current filter. When non-currency criteria change in automatic mode, resolve it again. An explicit currency choice enters manual mode and remains stable while other filters change. Charts requires a resolved currency before Apply/chart loading. If the selected filter contains no transactions, use `GetLastUsedCurrencyUseCase`; if that is also empty, use `CurrencyEntity.RUB`. Cover the complete fallback chain in reducer tests.
