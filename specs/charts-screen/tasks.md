...
- [x] Produce a screen/chart wireframe covering loading, empty, failure, and content states. (R4)
...
- [x] Define balance-total/category timeline entities in `core:entity`. (R4, R4a)
- [x] Add SQLDelight monthly category and chart queries with their required filter predicates. (R2, R4, R4a, R4b)
- [x] Apply the missing currency predicate to every existing SQL query backed by `FilterEntity`. (R2)
- [x] Extend `ReportDao`, report mappers, and `DefaultReportDao`; normalize sorting and missing buckets. (R4, R4a, R4b)
- [x] Add report API use cases, default implementations, and DI registrations. (R4, R4a, R4b)
...
- [x] Implement the Compose screen with title, filter action, chart cards, and all UI states. (R1, R4)
- [x] Add the Balance Total card and propagate the resolved filter to it with the other charts. (R4a, R5)
- [x] Add the monthly Income/Expense line chart filtered only by period and currency. (R4b, R5)
...
- [ ] Check compact screens, long category names, dark theme, accessibility labels, and empty datasets. (R4, R4a, R4b)
